...
$(function () {
    $(".msg-loader").hide();
    $.get("/api/patient/messsages/contacts", function(resp){
        $.each(resp.contacts, function(i,v){
            $("#elenco_contatti").append('<li onclick="loadMessages('+v['paz_id']+')">\n' +
                '                                    <span class="avatar"><img src=http://www.gravatar.com/avatar/'+ v['gravatar']+'" height="42" width="42" />\n' +
                '                                    </span>\n' +
                '                                <div class="chat-info">\n' +
                '                                    <h5 class="mb-0">'+v['full_name']+'</h5>\n' +
                '                                </div>\n' +
                '                            </li>');
        });
    });
...
function loadMessages(paz_id){
    $(".msg-loader").show();

    $.get("/api/messages/list/"+paz_id, function(resp){
        if(resp.status === 'error'){
            Swal.fire("Whoops!", "Qualcosa è andato storto, riprova o contattaci", "error");
            return;
        }

        $.each(resp.messages, function(i,v){
