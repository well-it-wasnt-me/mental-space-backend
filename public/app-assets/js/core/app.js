...
(function (window, document, $) {
  'use strict';
  $("a[url='"+$(location).attr('pathname')+"']").parent('li').addClass('active');
  var $html = $('html');
  var $body = $('body');
...
function searchPaz(){
  $.getJSON('/api/search/patient/' + $("#searchBarPaz").val(), function (data) {
    $.each(data, function(i,v){
      $(".search-list-main").append(
          '<li class="auto-suggestion">' +
          '<a class="d-flex align-items-center justify-content-between w-100" href="/pages/patients/detail/'+v['paz_id'] +'">' +
          '<div class="d-flex justify-content-start align-items-center">' +
          '<span>' +
          v['name'] + ' ' + v['surname'] + ' - ' + v['dob'] +
          '</span>' +
          '</div>' +
          '</a>' +
          '</li>');
